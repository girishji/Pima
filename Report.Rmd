...
+ 0.125(*pregnancy count*) + 3.966(*hereditary factor in the range [0, 1]*) + 
0.011(*age in years*) - 
0.024(*blood glucose X hereditary factor*).
...
specificity was 74%, and error rate was 22%. The area under
...
of the regressand (`Outcome`), compared to 258 positive instances (34.9%).
...
`r newthought('Logistic regression')` is the standard way to model binary outcomes. We model the probability that $DiabetesDiagnosis = 1$,
$$P(DiabetesDiagnosis_i = 1) = logit^{−1}(X_iβ),$$
under the assumption that the outcomes $DiabetesDiagnosis_i$ are independent given these probabilities.
...
```{r}
# Null model
round(min(sum(diabetes$Outcome == 1), 
