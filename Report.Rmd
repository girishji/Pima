...
```{r}
diabetes <- read.csv(url(str_c(
  "https://raw.githubusercontent.com/girishji/",
  "Pima/master/data/diabetes.csv")))
kable(diabetes[1:6, 1:6], 
      caption = 'A subset of data.')
```
...
```{r fig.cap = "Box plot of `Insulin` and `SkinThickness` showing spurious outliers.", cache=TRUE, fig.margin = TRUE, echo = F}
diabetes %>% 
  select_at(vars(Insulin, SkinThickness)) %>% 
  pivot_longer(everything(), names_to = 'Regressor', 
               values_to = 'Value') %>% 
  ggplot(aes(x = Regressor, y = Value, fill = Regressor)) +
  geom_boxplot() +
...
towards younger subjects. `Pregnancies` are also skewed towards fewer children.
...
```{r fig.cap = "Density plot of regressors.", cache=TRUE}
diabetes %>% 
  select(-Outcome) %>% 
  pivot_longer(everything(), names_to='Regressor', 
               values_to='Value') %>% 
...
```{r fig.cap = "Correlogram of regressors.", cache=TRUE}
diabetes %>% 
