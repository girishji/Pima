...
subtitle: "Development and Comparison"
...
To develop and validate an emperical equation to predict the onset of diabetes
...
BMI, capillary plasma glucose, a hereditory function, and
...
+ 0.125(*pregnancy count*) + 3.966(*hereditory factor in the range [0, 1]*) + 
0.011(*age in years*) - 
0.024(*blood glucose X hereditory factor*).
At 0.5 threshold for fitted values, equation's sensitivity was 88%, 
specificity was 58%, and accuracy was 78%. 
At 0.34 cut point for positive diagnosis, equation's sensitivity was 76%, 
specificity was 74%, and accuracy was 76%. The area under
ROC curve was 84%. In contrast, Smith et. al. report sensitivity and 
specificity
of 76%, and they did not report the area under ROC curve, even though
they reported the shape of the curve.
...
on par with more sophesticated
techniques.
...
`r newthought('Logistic regression')` is the standard way to model binary outcomes. We model the probability that $y = 1$,
...
```{marginfigure}
Graphical expression of the fitted logistic regression, $$P(diabetes) = logit^{−1}(-5.3501 + 0.0379 · Glucose).$$
```
...
`r newthought('Now we add DPF')`, the hereditory factor. Although diabetes mellitus is not
...
A quick search of all possible 
two-factor interactions based on reducing AIC
...
`r newthought('A binned residual plot')`^[p.97, Andrew Gelman and Jennifer Hill, _Data Analysis Using Regression and Multilevel/Hierarchical Models, 2006._] is better. 
...
To understand the deviation better, we bin and plot residuals with respect to 
...
to our fitted model). Enlarging the binned residuals plot 
for BMI we can 
see this situation better.

```{r}
arm::binnedplot(diabetes$BMI, 
                  residuals(fit.7, type = "response"), 
                  main = 'Binned residual plot of BMI')
```

The raising and falling nature of residuals suggests that 
a log transformation on `BMI` may help. Unfortunately, applying log 
...
```{r}
fit.8 <- glm(Outcome ~ Glucose + log(BMI) + Pregnancies +
               DPF + Age,
             diabetes %>% filter(BMI != 0), family = binomial(link="logit"))
arm::binnedplot(fitted(fit.8), 
                residuals(fit.8, type = "response"),
                main = 'Binned residual plot with log(BMI)')
```
...
```{r fig.fullwidth = TRUE}
base <- "Outcome ~ Glucose + BMI + Pregnancies + DPF + Age + "
par(mfrow = c(2, 3))
for (inter in c('Pregnancies:Age', 'Glucose:DPF', 
  ...
  arm::binnedplot(fitted(fit.temp), 
                  ...
                  main = str_c('After including ', inter))
}
```
...
that there are only 2 outliers close to 95% error boundry. 
This is a reasonably
good model.


```{r}
fit.9 <- glm(Outcome ~ Glucose + log(BMI) + Pregnancies +
               DPF + Age + Glucose:DPF,
             diabetes %>% filter(BMI != 0), family = binomial(link="logit"))
...
where, $$\small Xβ = -10.357 + 0.045(Glucose) + 0.082(BMI) + 0.125(Pregnancies) + 3.966(DPF) + 0.011(Age) + -0.024(Glucose:DPF)$$ 

The sign of the coefficent of `Glucose:DPF` is negative. We can
interpret the interaction between `Glucose` and `DPF` in two ways:

-  For every additional 10% (0.1) increase of hereditory proclivity to 
diabetes (`DPF`),
...
then hereditory plays less of a role in determining
the onset of diabetes.

Interpretation of other coefficents is already covered in previous
sections.

Summary of the model is as follows:

```{r warning=F}
formula <- Outcome ~ Glucose + BMI + Pregnancies + 
               DPF + Age + DPF:Glucose
...
error_rate(fitted(fit.f), diabetes$Outcome)
```
```{r}
# Null model
round(min(sum(diabetes$Outcome == 1), sum(diabetes$Outcome == 0)) /
  length(diabetes$Outcome) * 100, digits = 2)
```
...
Our model has AUC of 83.5%. This is a reasonable good value.
At the cut-off point of 0.5 for fitted values,
...
```{r message=F}
plot(pROC::roc(diabetes$Outcome, fitted(fit.f)), col = "#0042A0", 
...
```{r}
validate_error_rate <- function(threshold, iterations = 100) {
  return(c(1:iterations) %>% 
           map_dbl(~ {
             splitted <- split_data()
             predicted <- predict_diabetes(splitted$training, 
                                           splitted$test)
             error_rate(predicted, splitted$test$Outcome, 
                        threshold = threshold)
           }))
}
er <- validate_error_rate(threshold = 0.5)
er <- validate_error_rate(threshold = 0.36)
summary(er)
```
...
sensitivity <- c(1:100) %>% 
           map_dbl(~ {
             ...
             predicted <- predict_diabetes(splitted$training, 
                                           splitted$test)
             caret::sensitivity(
               factor(if_else(predicted >= 0.36, 1, 0)), 
               factor(splitted$test$Outcome))
           })
summary(sensitivity)
```
...
The precision is percentage of predicted truly relevant results of the total number of predicted relevant results
...
# 
# lrtest() for each addition, and summary()
```
