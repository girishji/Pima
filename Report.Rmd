...
`r newthought('To understand the deviation better')`, residuals are binned and
with respect to 
...
the influence produced by individual inputs, depending
...
diabetes (`DPF`),
a value of `r -0.1*round(fit.f$coefficients[[7]], digits=3)` is subtracted 
from the coefficient 
of `Glucose`. Subtracting from the (positive) coefficient of `Glucose`
...
`Diagnosis` owing to the log transformation. For every 10% increase
`BMI`, linear predictor increases by `r round(fit.f$coefficients[[3]],3)`x$\log(1.1)$=`r round(fit.f$coefficients[[3]] * log(1.1), 3)`. An inverse logit
function will reveal the increase in shadow (dependent) variable.
...
can be found on [github](https://github.com/girishji/Pima).
